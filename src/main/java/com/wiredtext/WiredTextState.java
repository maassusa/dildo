...
public class WiredTextState {
    ...
    // Configurable rate (in milliseconds)
    ...
    public static void scheduleNext() {
        long delay;
        if (overdrive) {
            delay = 300L; // 0.3 seconds
        } else {
            delay = minDelayMs + (long)(RANDOM.nextDouble() * (maxDelayMs - minDelayMs));
        }
    ...
    public static void trigger() {
        ...
        long duration = overdrive ? OVERDRIVE_DISPLAY_MS : DISPLAY_DURATION_MS;
        showUntil = System.currentTimeMillis() + duration;
        scheduleNext();
    }
